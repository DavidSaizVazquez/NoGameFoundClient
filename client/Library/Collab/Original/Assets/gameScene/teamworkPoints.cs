...
public class teamworkPoints : MonoBehaviour
{
    ...
    private GameObject[] globeList;
    private GameObject[] orderedGlobeList;

    private bool on=false;
    private List<bool> detectors;
    ...
    void Start()
    {
        ...
        globes = globeList.OrderBy(a => Guid.NewGuid()).ToList();
        
        foreach (GameObject globe in globes)
        {
            globe.SetActive(false);
        }
        detectors = new List<bool>();
        players = 1;
    ...
    void globesOnNpc(String globeNames)
    {
        activeClock = 0;
        on = true;
